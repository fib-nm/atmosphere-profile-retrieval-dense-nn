...
from preprocessing.atmrad.cpu.core.types import Tensor1D_or_3D, Tensor1D_or_2D, Tensor2D, cpu_float
from preprocessing.atmrad.cpu.core.const import *
import preprocessing.atmrad.cpu.core.math as math
from preprocessing.atmrad.cpu.core.common import at, cx
from preprocessing.atmrad.cpu.core import attenuation
from preprocessing.atmrad.cpu.core.static.water import vapor
import preprocessing.atmrad.cpu.core.integrate as integrate
from preprocessing.atmrad.cpu.core.multi import parallel
import numpy as np
...
class Atmosphere:
    def __init__(self, Temperature: Tensor1D_or_3D, Pressure: Tensor1D_or_3D,
        ...
        """
        self._T = math.as_tensor((Temperature[1:] + Temperature[:-1]) / 2)
        del Temperature

        self._P = math.as_tensor((Pressure[1:] + Pressure[:-1]) / 2)
        del Pressure

        if AbsoluteHumidity is None:
            AbsoluteHumidity = vapor.absolute_humidity(self._T, self._P, RelativeHumidity)
            del RelativeHumidity
        self._rho = math.as_tensor((AbsoluteHumidity[1:] + AbsoluteHumidity[:-1]) / 2)
        ...
        if altitudes is None:
            ...
            self._dh = np.asarray(dh, dtype=cpu_float)
            self._alt = np.cumsum([dh for _ in range(self._T.shape[-1])], dtype=cpu_float)   # self._alt - array
        else:
            assert self._T.shape[-1] == len(altitudes) - 1, 'lengths do not match'
            self._dh = np.diff(altitudes).astype(cpu_float)  # self._dh - array
            self._alt = np.asarray(altitudes[:-1], dtype=cpu_float)  # self._alt - array
        del altitudes
        del dh
